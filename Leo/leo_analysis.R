# this script reproduces propensity score estimation, exact matching on lagged outcomes, 
# full matching, balance diagnostics, and treatment effect estimation based on the original 
# analysis.R file
...
# script structure: 
  ...
  # 4. run the analysis
    ...
    # c. pre-matching balance diagnostics
    # d. estimate propensity score
    # e. exact matching
    # f. full matching 
      # i. main (em)
      # ii. restricted ratio (em.10)
      # iii. pair matching (pm)
    ...
    # l. regression models (treatment effect estimation with inference)
      ...
      # ii. lm(y ~ ndc + em.10) / weights = weight.10
      # iii. lm(y ~ ndc + pm) / weights = weight.pm
    # m. store results
  # 5. compile results
...
cfg_matchvars <- list(
  sets = list(mv1 = mv1, mv2 = mv2, mv3 = mv3, mv4 = mv4, mv5 = mv5, mv6 = mv6, mv7 = mv7, mv8 = mv8), 
  sets_for_outcome = matchvarlist # which set is used for which outcome
)
rm(mv1, mv2, mv3, mv4, mv5, mv6, mv7, mv8, matchvarlist)
